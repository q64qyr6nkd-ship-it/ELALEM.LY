...
try {
    $stmt = $conn->prepare("SELECT * FROM products WHERE category = 'juices' AND available = 1");
    $stmt->execute();
...
<title>عصائر  - حلويات العالم</title>
...
function autoUpdateProducts() {
    ...
    fetch("../updates.php?type=products_update&cat=juices")
        .then(res => res.json())
        .then(data => {
