...
try {
    $stmt = $conn->prepare("SELECT * FROM products WHERE category = 'cakes' AND available = 1");
    $stmt->execute();
...
<title> الكيكات - حلويات العالم</title>
...
function autoUpdateProducts() {
    ...
    fetch("../updates.php?type=products_update&cat=cake")
        .then(res => res.json())
        .then(data => {
