...
try {
    $stmt = $conn->prepare("SELECT * FROM products WHERE category = 'moroccan' AND available = 1");
    $stmt->execute();
...
<title>حلويات مغربية - حلويات العالم</title>
...
<section class="products-section">
    ...
    <div class="search-box">
        <input 
            type="text" 
            id="searchInput"
            placeholder="🔍 ابحث عن حلويات مغربية..."
...
function autoUpdateProducts() {
    ...
    fetch("../updates.php?type=products_update&cat=moroccan")
        .then(res => res.json())
        .then(data => {
