...
try {
    $stmt = $conn->prepare("SELECT * FROM products WHERE category = 'nuts' AND available = 1");
    $stmt->execute();
...
<title>اللوزيات  - حلويات العالم</title>
...
function autoUpdateProducts() {
    ...
    fetch("../updates.php?type=products_update&cat=nuts")
        .then(res => res.json())
        .then(data => {
