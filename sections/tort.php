...
try {
    $stmt = $conn->prepare("SELECT * FROM products WHERE category = 'tort' AND available = 1");
    $stmt->execute();
...
<title>التورتات  - حلويات العالم</title>
...
function autoUpdateProducts() {
    ...
    fetch("../updates.php?type=products_update&cat=tort")
        .then(res => res.json())
        .then(data => {
